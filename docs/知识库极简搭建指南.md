...
（这里以我的知识库作为模板）打开链接：[**linyuxuanlin/Wiki-digest_VuePress**](https://github.com/linyuxuanlin/Wiki-digest_VuePress)  
...
- `digest.wiki-power.com`：替换为你的自定义域名 `xx.com`
- `linyuxuanlin/Wiki-digest_VuePress`：替换为你的 GitHub 用户名和仓库名 `xxx/Wiki-book`
  - （如果仓库名称不为 `Wiki-book`，则需全局搜索并修改对应的仓库名）
- `main`：替换为 `main`. 狗屎 GitHub, 搞 nmd 政治正确，非要把新建仓库的 main 分支改名 main 分支
